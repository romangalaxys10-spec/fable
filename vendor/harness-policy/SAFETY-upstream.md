# Safety (upstream, verbatim)

Vendored from github.com/deepseek-ai/deepseek-harness `SAFETY.md` (MIT,
© DeepSeek AI) with their permission under that license. fable's harness
re-ships this notice because it applies equally to ANY plugin-loading agent
system — including ours. Read it, then read `POLICY.md` for what fable adds
on top (multi-phase audit, hash pinning, human approval flags).
...
Use DeepSeek Harness at your own risk. The software is provided without warranty under the [MIT License](https://github.com/deepseek-ai/deepseek-harness/blob/master/LICENSE). To the maximum extent permitted by applicable law, the authors and copyright holders are not responsible for damage to computers, loss of disclosure of data, loss of files, or other harm arising from use of the project.
